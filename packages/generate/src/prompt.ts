...
const ACTION_SHEET_STRUCTURE = [
  ...
  '64x64 pixel character',
  '4 rows x 4 columns grid layout on single image',
  'CHARACTER ONLY no props no desk no chair no objects in any frame',
...
const PROPS_STRUCTURE = [
  'prop sprites for a pixel RPG modern office',
...
const CHAIR_STRUCTURE = [
  'single desk chair sprite for a pixel RPG',
  '3/4 top-down perspective — the classic RPG overhead angle, same as desks tables and other furniture',
  'the chair is facing DIRECTLY AWAY from the camera — we see the BACK of the chair straight on, perfectly symmetrical left-right',
  'NOT rotated left or right — the chair faces straight up/north away from the viewer',
  'single item centered on transparent background',
  'no characters no people no desk',
].join(', ');
...
const TEXTURE_STRUCTURE = [
  ...
  'no objects no characters no props',
  ...
  'the floor should recede into the background and not compete with props or characters',
  'think simple muted flat surface not decorative',
].join(', ');

export type SheetType = 'walk' | 'action' | 'props' | 'object' | 'texture';
...
export function buildPrompt(description: string, type: SheetType): string {
  switch (type) {
    ...
    case 'props':
      return `${BASE_STYLE}, ${PROPS_STRUCTURE}, ${description}, consistent style across all items`;
    case 'object': {
      const isChair = /\bchair\b/i.test(description);
      const structure = isChair ? CHAIR_STRUCTURE : OBJECT_STRUCTURE;
      return `${BASE_STYLE}, ${structure}, ${description}`;
    }
    case 'texture':
